<div>
    <div
                ...
                @if ($isGlobalEpayEnabled)
                    <li wire:click="activeSetting('epay')" class="me-2">
                        <span @class([
                            ...
                            <span @class([
                                'flex w-3 h-3 me-3 rounded-full',
                                'bg-green-500' => $isEpayEnabled,
                                'bg-red-500' => !$isEpayEnabled,
                            ])></span>
                        </span>
                    </li>
                @endif

                @if ($isGlobalMollieEnabled)
                    <li wire:click="activeSetting('mollie')" class="me-2">
                        <span @class([
                            'inline-flex items-center gap-x-1 cursor-pointer select-none p-4 border-b-2 rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300',
                            'border-transparent' => $activePaymentSetting != 'mollie',
                            'active border-skin-base dark:text-skin-base dark:border-skin-base text-skin-base' =>
                                $activePaymentSetting == 'mollie',
                        ])>
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#C6D300" width="24" height="24">
                            ...
                            <span @class([
                                'flex w-3 h-3 me-3 rounded-full',
                                'bg-green-500' => $isMollieEnabled,
                                'bg-red-500' => !$isMollieEnabled,
                            ])></span>
                        </span>
                    </li>
                @endif

                @if ($isGlobalTapEnabled)
                    <li wire:click="activeSetting('tap')" class="me-2">
                        <span @class([
                            'inline-flex items-center gap-x-1 cursor-pointer select-none p-4 border-b-2 rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300',
                            'border-transparent' => $activePaymentSetting != 'tap',
                            'active border-skin-base dark:text-skin-base dark:border-skin-base text-skin-base' =>
                                $activePaymentSetting == 'tap',
                        ])>
                           <svg class="w-4 h-4 text-current" viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span @class([
                                'flex w-3 h-3 me-3 rounded-full',
                                'bg-green-500' => $isTapEnabled,
                                'bg-red-500' => !$isTapEnabled,
                            ])></span>
                        </span>
                    </li>
                @endif

                @if ($isGlobalFreshpayEnabled)
                    <li wire:click="activeSetting('freshpay')" class="me-2">
                        <span @class([
                            'inline-flex items-center gap-x-1 cursor-pointer select-none p-4 border-b-2 rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300',
                            'border-transparent' => $activePaymentSetting != 'freshpay',
                            'active border-skin-base dark:text-skin-base dark:border-skin-base text-skin-base' =>
                                $activePaymentSetting == 'freshpay',
                        ])>
                            <img src="{{ asset('images/logo-freshpay.png') }}" alt="FreshPay" class="object-contain w-14 h-14" />
                            @lang('modules.billing.freshpay')
                            <span @class([
                                'flex w-3 h-3 me-3 rounded-full',
                                'bg-green-500' => $isFreshpayEnabled,
                                'bg-red-500' => !$isFreshpayEnabled,
                            ])></span>
                        </span>
                    </li>
                @endif
                    ...
                    <li class="me-2">
                        <span wire:click="activeSetting('offline')" @class([
                            ...
                            <span @class([
                                'flex w-3 h-3 me-3 rounded-full',
                                'bg-green-500' => $hasAnyOfflinePaymentEnabled,
                                'bg-red-500' => !$hasAnyOfflinePaymentEnabled,
                            ])></span>
                        </span>
                    </li>
        ...
        @if($activePaymentSetting == 'epay')
            <form wire:submit="submitFormEpay">
                <div class="grid gap-6">

                    <div class="my-3">
                        <x-label for="epayStatus">
                            <div class="flex items-center cursor-pointer">
                                <x-checkbox name="epayStatus" id="epayStatus" wire:model.live='epayStatus' />
                                <div class="ms-2">
                                    @lang('modules.settings.enableEpay')
                                </div>
                            </div>
                        </x-label>
                    </div>
                    ...
                    @if ($epayStatus)
                        <div>
                            <x-label for="epayMode" :value="__('modules.settings.selectEnvironment')" required/>
                            <x-select id="epayMode" class="block w-full mt-1" wire:model.live="epayMode">
                                <option value="sandbox">@lang('app.test')</option>
                                <option value="live">@lang('app.live')</option>
                            </x-select>
                            <x-input-error for="epayMode" class="mt-2"/>
                        </div>

                        @if ($epayMode == 'live')
                            <div class="grid grid-cols-2 gap-x-4">
                                <div>
                                    <x-label for="epayClientId" :value="__('modules.settings.epayClientId')" required/>
                                    <x-input id="epayClientId" class="block w-full mt-1" type="text" wire:model='epayClientId'/>
                                    <x-input-error for="epayClientId" class="mt-2"/>
                                </div>

                                <div>
                                    <x-label for="epayClientSecret" :value="__('modules.settings.epayClientSecret')" required/>
                                    <x-input-password id="epayClientSecret" class="block w-full mt-1" type="text" wire:model='epayClientSecret'/>
                                    <x-input-error for="epayClientSecret" class="mt-2"/>
                                </div>
                            </div>

                            <div>
                                <x-label for="epayTerminalId" :value="__('modules.settings.epayTerminalId')" required/>
                                <x-input id="epayTerminalId" class="block w-full mt-1" type="text" wire:model='epayTerminalId'/>
                                <x-input-error for="epayTerminalId" class="mt-2"/>
                            </div>
                        @else
                            <div class="grid grid-cols-2 gap-x-4">
                                <div>
                                    <x-label for="testEpayClientId" :value="__('modules.settings.testEpayClientId')" required/>
                                    <x-input id="testEpayClientId" class="block w-full mt-1" type="text" wire:model='testEpayClientId'/>
                                    <x-input-error for="testEpayClientId" class="mt-2"/>
                                </div>

                                <div>
                                    <x-label for="testEpayClientSecret" :value="__('modules.settings.testEpayClientSecret')" required/>
                                    <x-input-password id="testEpayClientSecret" class="block w-full mt-1" type="text" wire:model='testEpayClientSecret'/>
                                    <x-input-error for="testEpayClientSecret" class="mt-2"/>
                                </div>
                            </div>

                            <div>
                                <x-label for="testEpayTerminalId" :value="__('modules.settings.testEpayTerminalId')" required/>
                                <x-input id="testEpayTerminalId" class="block w-full mt-1" type="text" wire:model='testEpayTerminalId'/>
                                <x-input-error for="testEpayTerminalId" class="mt-2"/>
                            </div>
                        @endif
        ...
        @if($activePaymentSetting == 'mollie')
            <form wire:submit="submitFormMollie">
                <div class="grid gap-6">
                    ...
                    @if ($mollieStatus)
                        <div>
                            <x-label for="mollieMode" :value="__('modules.settings.selectEnvironment')" required/>
                        ...
                        @if ($mollieMode == 'live')
                            <div>
                                <x-label for="liveMollieKey" :value="__('modules.settings.mollieLiveKey')" required/>
                        ...
                        @else
                            <div>
                                <x-label for="testMollieKey" :value="__('modules.settings.mollieTestKey')" required/>
        ...
        @if ($activePaymentSetting == 'serviceSpecific')
            @if ($isRazorpayEnabled || $isStripeEnabled || $isFlutterwaveEnabled || $isPaypalEnabled || $isPayfastEnabled || $isPaystackEnabled || $isXenditEnabled || $isEpayEnabled || $isMollieEnabled || $isTapEnabled || $isFreshpayEnabled || $hasAnyOfflinePaymentEnabled || $enableQrPayment)
