...
class FrontDetailSeeder extends Seeder
{
    ...
    {
       $languageSetting = LanguageSetting::where('active', 1)->first();

        if ($languageSetting->active == 1) {
            $modelMappings = [
                FrontDetail::class => 'detail',
                FrontFeature::class => 'features',
                FrontReviewSetting::class => 'review',
                FrontFaq::class => 'frontFaq',
                Contact::class => 'contact'
            ];

            foreach ($modelMappings as $model => $method) {
                if (!$model::where('language_setting_id', $languageSetting->id)->exists()) {
                    $this->$method($languageSetting->id);
                }
            }
    ...
    public function features($languageId)
    {
        ...
        $features = [
            ...
            [
                ...
                'image' => '<svg class="size-6 transition duration-75 text-skin-base dark:text-skin-base" fill="currentColor"
